...
class MemosAIClient:
    ...
    Routes all requests through the Memos Gateway using the master key,
    while tracking usage via the user's Activation License Key.
    """
    ...
    def __init__(self):
        ...
        # Production Gateway URL (Mocked using Groq's OpenAI-compatible endpoint)
        self.gateway_url = os.getenv('MEMOS_AI_GATEWAY', 'https://api.groq.com/openai/v1/chat/completions')
        
        # The Master Developer Key used to pay for all users' inference
        self.master_key = getattr(settings, 'GROQ_API_KEY', '') or os.getenv('MASTER_GROQ_API_KEY', '')
        
        # Default model for the gateway
        self.default_model = getattr(settings, 'GROQ_MODEL', 'mixtral-8x7b-32768')
    ...
    def _get_headers(self):
        ...
        return {
            ...
            "X-Memos-License": self.license_key, # Track user usage
    ...
    def _ensure_active(self):
        ...
        if not self.master_key:
            raise ValueError("AI gateway key is missing. Set GROQ_API_KEY (or MASTER_GROQ_API_KEY) in backend/.env and restart the backend.")

    def chat_completion(self, messages, model=None, temperature=0.5, max_tokens=1024, response_format=None, stream=False):
        """Generic chat completion through the Memos Gateway."""
        self._ensure_active()
        ...
        try:
            ...
            if stream:
                return response # Return raw requests response for streaming
            else:
                data = response.json()
