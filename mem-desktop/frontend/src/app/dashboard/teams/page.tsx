...
import { TeamIcon, GlobeIcon, LockIcon, LinkIcon, InviteIcon, OwnerIcon, EditorIcon, ViewerIcon } from '@/components/TeamIcons';
import { Team } from '@/lib/teamApi';

export default function TeamsListPage() {
  const { teams, loading, fetchTeams, deleteTeam } = useTeam();
  ...
  const getRoleIcon = (role: string) => {
    if (role === 'owner') return <OwnerIcon size={12} />;
    if (role === 'editor') return <EditorIcon size={12} />;
    return <ViewerIcon size={12} />;
  };

  const getVisibilityIcon = (visibility: string) => {
    if (visibility === 'private') return <LockIcon size={14} />;
    if (visibility === 'link') return <LinkIcon size={14} />;
        ...
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="panel-sub">Collaborative workspaces</p>
            </div>
          </div>
          <button
